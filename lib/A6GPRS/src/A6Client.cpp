...
int A6CLIENT::connect(const char *path, uint16_t port)
{
  ...
  {
	  a6instance->debugWrite("Close stale connection");
	  stop();
	  a6instance->CIPstatus = a6instance->getCIPstatus();	  
  }
...
//    Serial.println(TempBuf);
    ...
    if ( a6instance->waitresp("CONNECT OK",7500))  // must be leaa than 8 secs as in watchdog
    {
...
bool A6CLIENT::TxBegin()
{
  ...
  return a6instance->waitresp(">",1000);
}
...
size_t A6CLIENT::write(const void *buf, uint16_t len, uint32_t flags = 0)
{
...
int A6CLIENT::read(void *buf, uint16_t len, uint32_t flags = 0)
{
  unsigned j=0;
  int data,count;
  while (j++ < len)
