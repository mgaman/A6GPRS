...
A6CALL::~A6CALL(){}
void A6CALL::Parse(char buffer[],unsigned length)
{
  int parm;
  char *start;
  if (length == 1)
  ...
  else if (strncmp(buffer,"+CIEV:",6) == 0)
  {
    ...
    else
    {
      ...
      OnPhoneEvent(UNKNOWN_EVENT,strlen(buffer));
    }
  }
  ...
  else if (strncmp(buffer,"+CREG:",6) == 0)
  {
    start = strchr(buffer,' '); // point to begining of string
    parm = atoi(start);
    OnPhoneEvent(CREG_ARRIVED_EVENT,parm);
  }
  else if (strncmp(buffer,"+CGREG:",7) == 0)
  {
    start = strchr(buffer,' '); // point to begining of string
    parm = atoi(start);
...
bool A6CALL::sendSMS(char addr[],char text[],enum eDCS alphabet)
{
  bool rc = false;
//  int i,pdulength;
//  char tt[8];
