...
A6GPRS::A6GPRS(Stream &comm,unsigned cbs,unsigned maxmessagelength){
  ...
  modemmessage = new char[maxmessagelength];
...
static char const *statusnames[] = {"IP INITIAL","IP START","IP CONFIG","IP IND","IP GPRSACT","IP STATUS","TCP/UDP CONNECTING","IP CLOSE","CONNECT OK"};
...
bool A6GPRS::begin(char const *apn,char const *user,char const *pwd)  // apn, username, password
{
  bool rc = false;
  RXFlush();
...
bool A6GPRS::begin(char const *apn)  // apn
{
  return begin(apn,"","");
}
 ...
 */
char *A6GPRS::Parse(unsigned *length,bool *payload)
{
  char *mm = 0;
  *length = 0;
  bool alldone = false;
...
//            Serial.println(F("Server closed connection"));
